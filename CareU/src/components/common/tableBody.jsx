...
class TableBody extends Component {
	...
	renderColumn = (arrValues, columns, item, idxRow) => {
		...
		for (var i = 1; i < _.size(arrValues); i++) {
			...
			if (columns[i] !== undefined && columns[i].content) {
				arrCols.push(
					<td key={this.createKey(item, columns[i])}>{columns[i].content(item, idxRow, arrValues[0])}</td>
				);
			}
		}
		...
		for (var j = length; j < _.size(columns); j++) {
			if (columns[j] !== undefined && columns[j].content) {
				arrCols.push(
					<td key={this.createKey(item, columns[j])}>{columns[j].content(item, idxRow, arrValues[0])}</td>
				);
			}
		}
		return arrCols;
	...
	renderCell = (item, column, conspan, index, idxRow) => {
		...
		if (column.content && column.rowspan !== undefined) {
			return (
				<td key={this.createKey(item, column)}>{column.content(item, idxRow, arrValues[0])} </td>);
		}
