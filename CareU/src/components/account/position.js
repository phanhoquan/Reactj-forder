...
import positions from '../../services/accountPosition';
...
class Position extends Component {
	constructor(props) {
		...
		this.state = {
			...
			openModalEdit: false,
			error: false,
			openModalWarningPositionUsing: false,
			isClick: false
		}
	}
	...
	_renderData = async (id, page, pageSz) => {
		try {
			let dataResult = await positions.getListPositions(id, page, pageSz);
			let data = [...dataResult.data.data.positions];
	...
	handleEdit = (dataEdit) => {
		this.setState({
			...
			this.onHandleChange({ openModalEdit: true });
		});
	}

	handleDelete = async () => {
		try {
			let response = await positions.deletePositions(this.state.id);
			console.log(response.data.data.length)
			if(response.data.data.length !== 0)
			{
				this.onHandleChange({ openModalDelete: false});
				setTimeout(() => { this.onHandleChange({ openModalWarningPositionUsing: true }); }, 500);
	...
	hanleSubmit = async () => {
		...
		{
			try {
				await positions.addPositions(data);
				...
				toast.error('위치가 이미 있습니다.');
			}
		}else {
			toast.error('직위가 필요합니다.');
		}
		setTimeout(() => {
			this.setState({
				isClick: false
	...
	handleSubmitEdit = async () => {
		...
		try {
			let response = await positions.updatePositions(this.state.dataEdit);
	...
	renderError = () => {
		if (this.state.error) {
			return (
				<span className="help-block text-danger">직책을 입력하세요</span>
			);
		}
	}
		...
		return (
			<div>
				...
				<Modal className='modalPopup w-55'
					...
					onHide={() => this.onHandleChange({ openModalEdit: false, error: false })}
				>
					...
					<div className="text-center">
						...
						<button
							...
							onClick={() => this.onHandleChange({ openModalEdit: false, error: false })}
						>취소</button>
					</div>
				</Modal>
				...
				<Modal className='modalPopup w-45'
					id="openModalWarningPositionUsing"
					show={this.state.openModalWarningPositionUsing}
					onHide={() => this.onHandleChange({ openModalWarningPositionUsing: false })}
				>
					<Modal.Header closeButton>
					</Modal.Header>
					<Modal.Title>웹 사이트로부터의 통지</Modal.Title>
					<div>
						<p className="text-center pt-16">사용중인 부서를 삭제 불 가능합니다</p>
						<div className="button-footer text-center">
							<button
								className="btn btn-secondary"
								onClick={() => this.onHandleChange({ openModalWarningPositionUsing: false })}
							>확인</button>
						</div>
					</div>
				</Modal>
