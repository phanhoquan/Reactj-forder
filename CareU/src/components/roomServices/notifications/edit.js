...
import { isNumberKey, getCaregiverNumber, getPrefixWard1, getPrefixRoom1, checkAuth } from '../../../services/functionService';
import { getNotification, updateNotification, getCountEmergency, setExpired } from '../../../services/notification';
import ReactDOM from 'react-dom';
import {apiChangeNotice} from "../../../services/roomStatus";
...
class EditNotification extends Component {
	constructor(props) {
		...
		this.state = {
			...
			checkErrorAreaRoom: false,
			checkErrorWorkingDate: false,
			checkErrorCaregiver: false,
			checkErrorCareWork: false,
		}
	}
	...
	async componentWillMount() {
		...
		if (this.props.match.params.id != undefined) {
			this._renderData(this.props.match.params.id);
		}
	};

	_renderData = async (id) => {
		let countEmergency = await getCountEmergency(this.props.master_id);
		/*Get areas of hospital*/
		...
		if (response.data.data != undefined) {
			...
			this.setState({
				defaultRoomId: notification.room_id,
				...
				defaultNotification: {
					...
					area_id: notification.room.area_id,
					room_id: notification.room_id,
					notice_type: notification.notice_type,
					salary: notification.salary.toString().replace(/(-?\d+)(\d{3})/, "$1,$2"),
					salary_bonus: notification.salary_bonus.toString().replace(/(-?\d+)(\d{3})/, "$1,$2"),
					count_patient: notification.count_patient,
					count_patient_plus: notification.count_patient_plus,
					work_type: notification.work_type,
					start_day: notification.start_day != null ? moment(notification.start_day).format("YYYY-MM-DD HH:mm") : '',
					end_day: notification.end_day != null ? moment(notification.end_day).format("YYYY-MM-DD HH:mm") : '',
					count_male_aide: notification.count_male_aide,
					count_female_aide: notification.count_female_aide,
					count_other_aide: notification.count_other_aide,
					count_couple_aide: notification.count_couple_aide,
					care_suction: notification.care_suction,
					care_feeding: notification.care_feeding,
					care_physiotherapy: notification.care_physiotherapy,
					care_dialysis: notification.care_dialysis,
					care_respiratory: notification.care_respiratory,
					material_tv: notification.material_tv,
					material_wifi: notification.material_wifi,
					material_electric_bed: notification.material_electric_bed,
					material_care_bed: notification.material_care_bed,
					material_locker: notification.material_locker,
					material_restroom: notification.material_restroom,
					facility_sank: notification.facility_sank,
					facility_washing: notification.facility_washing,
					facility_microwave: notification.facility_microwave,
					facility_restaurant: notification.facility_restaurant,
					note: notification.note != null ? notification.note : '',
					created_at: notification.created_at,
					name_master: notification.user.full_name
				},
				default: {
					area_id: notification.room.area_id,
					room_id: notification.room_id,
				...
				defaultArea: {
					...
					id: notification.room.area.id,
					name: notification.room.area.name
				},
				errorAreaRoom: TYPE_ERROR.VALID,
				defaultNoticeType: notification.notice_type
			});

			/*Get rooms by area*/
			this.changeSelect(notification.room.area.id, 'area', true);
			this.changeSelect(notification.room_id, 'room', true);
		}
	}
	...
	changeSelect = async (id, name, checkValid) => {
		switch (name) {
			case "area":
				...
				await this.setState({
					...
					if (id != '') {
						...
						var errorAreaRoom = TYPE_ERROR.VALID;
						if (id != this.state.defaultNotification.area_id) {
							errorAreaRoom = TYPE_ERROR.ERROR;
						}

						this.setState({
							rooms: this.mapListToSelectData(response.data.data),
							errorAreaRoom: errorAreaRoom,
							messageAreaRoom: "지역을 선택하십시오."
						});
					} else {
						this.setState({
							errorAreaRoom: checkValid == true ? TYPE_ERROR.ERROR : '',
							messageAreaRoom: "지역을 선택하십시오.",
							checkErrorAreaRoom: true
						})
					}					
				})
				break;
				...
				await this.setState({
					...
					if (id != '') {
						...
						this.setState({
							...
							errorAreaRoom: checkValid === true ? TYPE_ERROR.VALID : '',
						});

						if (id != this.state.defaultRoomId) {
		...
		if (this.state.default.area_id !== this.state.defaultNotification.area_id || this.state.default.room_id !== this.state.defaultNotification.room_id) {
			this.setState({
				checkErrorAreaRoom: true
			});
		}
	...
	checkCaregiver = () => {
		...
		if (countMaleAide > 20 || countFemaleAide > 20 || countOtherAide > 20) {
			...
			if (countMaleAide == 0 && countFemaleAide == 0 && countOtherAide == 0 && (countCoupleAide == 0 || countCoupleAide == 'false')) {
				this.setState({
					errorCaregiver: TYPE_ERROR.ERROR
				})
			} else {
	...
	checkWokingDates = (workingType) => {
		...
		if (workingType == TYPE_WORKING.SHORT && this.state.defaultNotification.end_day.length === 0) {
			this.setState({
				errorWorkingDate: TYPE_ERROR.ERROR,
				messageWorkingDate: '필수 입력 사항입니다'
			})
		}else {
			if (workingType == TYPE_WORKING.LONG && this.state.defaultNotification.start_day.length === 0) {
				this.setState({
					errorWorkingDate: TYPE_ERROR.ERROR,
					messageWorkingDate: '필수 입력 사항입니다'
				})
	...
	checkValidateWorkTime() {
		...
		if (workType == TYPE_WORKING.LONG) {
			if (startDate == undefined || startDate <= now) {
				...
				if (this.state.default.start_day !== this.state.defaultNotification.start_day) {
					this.setState({
						errorWorkingDate: TYPE_ERROR.ERROR,
			...
			if (startDate == undefined || endDate == undefined || startDate <= now || endDate <= now || startDate >= endDate) {
				...
				let errorWorkingDate = TYPE_ERROR.VALID;
				if (startDate == undefined || endDate == undefined) {
					messageWorkingDate = '필수 입력 사항입니다';
					errorWorkingDate = TYPE_ERROR.ERROR;
				} else if (startDate <= now || endDate <= now) {
					if(startDate <= now && this.state.default.start_day !== this.state.defaultNotification.start_day)
					{
						messageWorkingDate = '시작일은 현재보다 켜야합니다';
						errorWorkingDate = TYPE_ERROR.ERROR;
					}
					if(endDate <= now)
					{
					...
					if (this.state.default.end_day !== this.state.defaultNotification.end_day && startDate >= endDate ) {
						messageWorkingDate = '종료일은 시작일보다 켜야합니다';
						errorWorkingDate = TYPE_ERROR.ERROR;
					}
				}
				...
				this.setState({
					errorWorkingDate: errorWorkingDate,
					messageWorkingDate: messageWorkingDate
				})
			} else {
				this.setState({
		...
		if (this.state.default.start_day !== this.state.defaultNotification.start_day || this.state.default.end_day !== this.state.defaultNotification.end_day) {
			this.setState({
				checkErrorWorkingDate: true
			});
		}
	}
	...
	setDeadline = async (e) => {
		let data = { "status": STATUS_EXPIRED };
		let response = await setExpired(this.props.match.params.id, data);
	...
	renderTerminateButton = () => {
		return (
			<div className="text-left d-inline-block">
				<button className="btn btn-secondary" onClick={this.setDeadline}>마감</button>
			</div>
		)
	}
	...
	submitForm = async (e) => {
		...
		if (this.isFormUpdateValid()) {
			...
			if(this.state.defaultNotification.work_type == TYPE_WORKING.LONG)
			{
				this.state.defaultNotification.end_day = '';
			}

			//Update a notification
			let response = await updateNotification(this.props.match.params.id, this.state.defaultNotification);
			// let param = this.props.match.params;
			// if(param && param.type !== 0) {
			// 	let response = await apiChangeNotice(param.type, this.props.master_id, '끝');
	...
	renderDate = (type, date) => {
		const shortTermDisplay = this.state.defaultNotification.work_type == TYPE_WORKING.LONG ? "d-none" : "d-inline-block";
		...
		if (date.length !== 0) {
			return (
				<Datetime
					...
					onChange={evt => this.handleChangeDatetime(evt, type)}
					timeFormat="HH:mm"
				/>
			);
		}
	...
	render() {
		const btnSumitStatus = this.isFormUpdateValid() ? BTN_SUBMIT_VALID : BTN_SUBMIT_INVALID;
		const btnSubmitName = BTN_SUBMIT_UPDATE;
		...
		//let selectedArea = this.state.areas.filter(item => item.value == this.state.defaultArea.id)[0];
		let selectedRoom = this.state.rooms.filter(item => item.value == this.state.defaultNotification.room_id)[0];
		if (selectedRoom === undefined) {
			selectedRoom = { value: '', label: '선택' };
		}
	
		let createdAt = moment(this.state.defaultNotification.created_at).format("YYYY-MM-DD HH:mm");
						...
						<tbody>
							<tr>
								...
								<td className='position-relative d-flex justify-content-start border-bottom-0'>
									<Select
										...
										className="mr-2"
										placeholder=""
										onChange={(e) => this.changeSelect(e.value, "area", true)}
									...
									<Select
										ref="room_id"
										placeholder=""
										onChange={(e) => this.changeSelect(e.value, "room", true)}
										options={this.state.rooms}
										blurInputOnSelect={true}
										isSearchable={false}
										value={selectedRoom}
									/>
									{this.state.checkErrorAreaRoom ? this.renderError(this.state.errorAreaRoom, this.state.messageAreaRoom) : ''}
								</td>
									...
									<div className="custom-radio">
										<label className="radio-wrapper">
											<input
												...
												checked={this.state.defaultNotification.notice_type == NOTICE_TYPE.NORMAL ? true : false}
												onChange={() => this.changeNoticeType(NOTICE_TYPE.NORMAL)}
											/>
											<span className="checkradio" />
											<span className="text">일반</span>
										</label>
										<label className={this.state.countEmergency === 5 && this.state.defaultNoticeType == NOTICE_TYPE.NORMAL ? 'radio-wrapper not-allowed' : "radio-wrapper"}>
											<input
												type="radio"
												disabled={this.state.countEmergency === 5 && this.state.defaultNoticeType == NOTICE_TYPE.NORMAL ? true : false}
												name={NOTICE_TYPE_OPTION}
												defaultValue={NOTICE_TYPE.EMERGENCY}
												checked={this.state.defaultNotification.notice_type == NOTICE_TYPE.EMERGENCY ? true : false}
									...
									<div className="custom-radio">
										<label className="radio-wrapper">
											<input
												...
												defaultValue={1}
												checked={this.state.defaultNotification.work_type == 1 ? true : false}
										...
										<label className="radio-wrapper">
											<input
												...
												name={WORK_TYPE_OPTION}
												defaultValue={2}
												checked={this.state.defaultNotification.work_type == 2 ? true : false}
							...
							<tr>
								...
								<td className="custom-datetime position-relative d-flex align-items-center border-none">
									...
									<Datetime
										...
										ref="startDate"
										inputProps={{ placeholder: '시 자동 입력됩니다', readOnly: true }}
										value={this.state.defaultNotification.start_day}
									...
									<Datetime
										...
										ref="endDate"
										inputProps={{ placeholder: '시 자동 입력됩니다', readOnly: true }}
										value={this.state.defaultNotification.end_day}
										onBlur={(e) => this.checkValidateWorkTime()}
										onChange={e => this.handleChangeDatetime(e, 2)}
										timeFormat="HH:mm"
										dateFormat={'YYYY-MM-DD'}
									/>
									{this.state.checkErrorWorkingDate ? this.renderError(this.state.errorWorkingDate, this.state.messageWorkingDate) : ''}
								</td>
							</tr>
							<tr>
								...
								<td className="position-relative d-flex justify-content-start border-bottom-0 ">
									...
									<div className="handling-problem d-flex align-items-center">
										<label className="checkbox-wrapper ml-4">
											<input
												...
												checked={this.state.defaultNotification.count_couple_aide}
									...
									{this.state.checkErrorCaregiver ? this.renderError(this.state.errorCaregiver, "필수 입력 사항이며 값이 20보다 작이나 20만큼 됩니다") : ''}
								</td>
							</tr>
							<tr>
								...
								<td className="position-relative d-flex justify-content-start border-bottom-0 ">
									<div className="handling-problem custom-slectoption w-35 w-12 border-bottom-0">
										...
										<input
											...
											defaultValue={this.state.defaultNotification.care_suction != 0 ? this.state.defaultNotification.care_suction : ""}
										/>
										<span className="text pl-1">명</span>
									</div>
									<div className="handling-problem custom-slectoption w-35 w-12 border-bottom-0">
										...
										<input
											...
											defaultValue={this.state.defaultNotification.care_feeding != 0 ? this.state.defaultNotification.care_feeding : ""}
										/>
										<span className="text pl-1">명</span>
									</div>
									<div className="handling-problem custom-slectoption w-35 w-12 border-bottom-0">
										...
										<input
											...
											defaultValue={this.state.defaultNotification.care_physiotherapy != 0 ? this.state.defaultNotification.care_physiotherapy : ""}
										/>
										<span className="text pl-1">명</span>
									</div>
									<div className="handling-problem custom-slectoption w-35 w-12 border-bottom-0">
										...
										<input
											...
											defaultValue={this.state.defaultNotification.care_dialysis != 0 ? this.state.defaultNotification.care_dialysis : ""}
										/>
										<span className="text pl-1">명</span>
									</div>
									<div className="handling-problem custom-slectoption w-35 w-12 border-bottom-0">
										...
										<input
											...
											defaultValue={this.state.defaultNotification.care_respiratory != 0 ? this.state.defaultNotification.care_respiratory : ""}
										/>
										<span className="text pl-1">명</span>
									</div>
									{this.state.checkErrorCareWork ? this.renderError(this.state.errorCareWork, "필수 입력 사항이며 값이 ") : ''}
								</td>
							</tr>
							<tr>
								<th>병실 내 편의시설</th>
								<td className="custom-checkbox position-relative">
									<label className="checkbox-wrapper">
										<input
											...
											checked={this.state.defaultNotification.material_tv}
									...
									<label className="checkbox-wrapper">
										<input
											...
											className="roomFacility"
											checked={this.state.defaultNotification.material_wifi}
									...
									<label className="checkbox-wrapper">
										<input
											...
											className="roomFacility"
											checked={this.state.defaultNotification.material_electric_bed}
									...
									<label className="checkbox-wrapper">
										<input
											...
											className="roomFacility"
											checked={this.state.defaultNotification.material_care_bed}
									...
									<label className="checkbox-wrapper">
										<input
											...
											className="roomFacility"
											checked={this.state.defaultNotification.material_locker}
									...
									<label className="checkbox-wrapper">
										<input
											...
											className="roomFacility"
											checked={this.state.defaultNotification.material_restroom}
								...
								<td className="custom-checkbox">
									<label className="checkbox-wrapper">
										<input
											...
											checked={this.state.defaultNotification.facility_sank}
									...
									<label className="checkbox-wrapper">
										<input
											...
											className="hospitalFacility"
											checked={this.state.defaultNotification.facility_microwave}
									...
									<label className="checkbox-wrapper">
										<input
											...
											className="hospitalFacility"
											checked={this.state.defaultNotification.facility_washing}
									...
									<label className="checkbox-wrapper">
										<input
											...
											className="hospitalFacility"
											checked={this.state.defaultNotification.facility_restaurant}
							...
							<tr>
								...
								<td className="position-relative">
									<input type="text"
										className="form-control col-2 m-0 created"
										name="regisDateTime"
										ref="regisDateTime"
							...
							<tr>
								<th>작성자</th>
								<td>
									{this.state.defaultNotification.name_master}
								</td>
							</tr>
							<tr>
								<th>참고사항</th>
								<td>
									...
									<textarea
										...
										ref="note"
										rows="3"
										value={this.state.defaultNotification.note}
...
export default withRouter(connect(mapStateToProps)(EditNotification));